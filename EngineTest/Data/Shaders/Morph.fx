...
V2PPLMORPH VSMain_MORPH(A2VMORPH input)
{
	
	// initilize output
	V2PPLMORPH output = (V2PPLMORPH)0;
	
	
	
	// get local position for transform
	
	float4 localPos0 = float4(input.position0.xyz,1.0f);
	float4 worldPos0 = mul(localPos0,WorldMatrix);
	float4 localPos1 = float4(input.position1.xyz,1.0f);
	float4 worldPos1 = mul(localPos1,WorldMatrix);
	
	// stuff for weight
	float4 LerpedPos = lerp(worldPos0,worldPos1,Weight);
	float3 norm = lerp(input.normal0,input.normal1,Weight); 
	
	float4 viewPos = mul(LerpedPos,ViewMatrix);
	float4 projPos = mul(viewPos,ProjMatrix);
	
	//calc dir to viewer
	float3 dirToViewer = ViewPosition.xyz - LerpedPos.xyz;
	dirToViewer = normalize(dirToViewer);
	
	// calc dir to light
	float3 dirToLight = LightPosition.xyz - LerpedPos.xyz;
	dirToLight = normalize(dirToLight);
	
	output.position		= projPos;
	output.normal		= mul(norm, WorldMatrix);
	output.dirToLight	= dirToLight;
...
P2F PSMain_PPLMORPH(V2PPLMORPH input)
{
// initilize output
	P2F output = (P2F)0;
...
technique Morph
{
	pass P0
	{
		vertexshader	= compile vs_2_0 VSMain_MORPH();
		pixelshader		= compile ps_2_0 PSMain_PPLMORPH();
